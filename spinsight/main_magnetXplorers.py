...
TISSUES = {
    ...
    'perotineum': {'PD': 1.0, 'FF': .00, 'T1': {1.5: 1500, 3.0: 1500}, 'T2': {1.5:   10, 3.0:   10}, 'hexcolor': 'ff8080'},
    'black':      {'PD': 1.0, 'FF': .00, 'T1': {1.5: 100000, 3.0: 100000}, 'T2': {1.5:   0.01, 3.0:   0.01}, 'hexcolor': '000000'} # hardcoding that black has no MR signal
    }
...
PHANTOMS = {
    ...
    'Arkynox_front': {'FOV': (360, 322), 'matrix': (601, 601), 'referenceTissue': 'kidneyCor'}, #front view of the phantom we designed
    'Arkynox_side': {'FOV': (360, 322), 'matrix': (601, 601), 'referenceTissue': 'kidneyCor'} #side view of the phantom we designed
}
...
def getSubpaths(pathString, scale):
    ...
    for entry in pathString.strip().replace(',', ' ').split():
        if command.upper() == 'Z': # new subpath
            ...
            subpath = []
        if entry.upper() in commands or entry.lower()== 'm': # handling of "Move to" command in relative 'm' instead of just in absolute values ('M')
            if entry == 'm' and subpath:
                subpaths.append(preparePath(subpath))
                subpath = []
            if entry.upper() in supportedCommands or entry.lower() == 'm':
                command = entry
            else:
            ...
            if command.upper() == 'H':
                ...
                x, y = 0, entry
            elif command.upper() in 'ML' or command.lower() == 'm':
                if x is None:
                    x = entry
                else:
                    y = entry
...
def readSVG(inFile):
    CUSTOM_COLOR_FOR_NONE = '000000'
    polygons = []
    for path in ET.parse(inFile).iter('{http://www.w3.org/2000/svg}path'): 
        style = path.attrib.get('style', '')
        if not style.startswith('fill:'):
            continue  # ignores if the style does not start with "fill:"
        fill_value = style[6:12]  # takes the color after 'fill:'
        if fill_value.lower().startswith('one'):
            hexcolor = CUSTOM_COLOR_FOR_NONE
        else:
            hexcolor = style[6:12]  # the 6 characters following 'fill:'
...
class MRIsimulator(param.Parameterized):
    ...
    fieldStrength = param.ObjectSelector(default=3, label='B0 field strength [T]')
    parameterStyle = param.ObjectSelector(default='Matrix and Pixel BW', label='Parameter Style', precedence=-1)
    
    FatSat = param.Boolean(default=False, label='Fat saturation', precedence=-1)
    TR = param.Selector(default=1000, label='Parameter 1 - TR [msec]')
    TE = param.Selector(default=60, label='Parameter 2 - TE [msec]')
    ...
    FOVP = param.Number(default=400, precedence=2, label='FOV x [mm]')
    FOVF = param.Number(default=500, precedence=2, label='FOV y [mm]')
    ...
    matrixP = param.Selector(default=64, precedence=4, label='Acquisition matrix x')
    matrixF = param.Selector(default=64, precedence=4, label='Acquisition matrix y')
    ...
    sequence = param.ObjectSelector(default=SEQUENCES[1], precedence=1, label='Pulse sequence')
    ...
    showFOV = param.Boolean(default=False, label='Show FOV', precedence=-1)
    ...
    kspaceExponent = param.Number(default=0.2, step=.01, label='k-space exponent',precedence=-1)
    ...
    def updateBWbounds(self):
        ...
        if isGradientEcho(self.sequence):
            ...
            readStartToTE -= max(
                self.boards['frequency']['objects']['read prephaser']['dur_f'] + self.readout_risetime, # consider maximum dur+risetime, not only current (difficult!)
                minPhaserTime,
                self.boards['slice']['objects']['slice select excitation']['riseTime_f'] + self.boards['slice']['objects']['slice select rephaser']['dur_f'])
...
def getApp(darkMode=True, settingsFilestem=''):
    ...
    title = 'MagnetXplorers MRI simulator'
    ...
    postprocParams = pn.panel(simulator.param, parameters=['homodyne', 'doApodize', 'apodizationAlpha', 'doZerofill'], name=' ')
    ...
    # @todo IN THE FOLLOWING LINE PLEASE REPLACE THE "X" WITH THE CORRECT PATH TO THE ARKYNOX_2X_CLEAN_CENTER.SVG FILE IN ORDER TO DISPLAY THE REFERENCE PICTURE OF THE ARKYNOX IN THE SIMULATOR INTERFACE.
    dmapKspace = pn.pane.Image(r"X")
